...
export function toolDescriptors() {
  ...
  return [
    ...
    { name: 'git.branch', description: 'List branches (no args) or create one ({name}).', inputSchema: s({ name: { type: 'string' } }) },
    { name: 'git.checkout', description: 'Switch branches (or create with create=true).', inputSchema: s({ branch: { type: 'string' }, create: { type: 'boolean' } }, ['branch']) },
    { name: 'git.push', description: 'Push to a remote (optionally sets upstream).', inputSchema: s({ remote: { type: 'string' }, branch: { type: 'string' }, setUpstream: { type: 'boolean' } }) },
    { name: 'git.pull', description: 'Pull from a remote (ff-only by default — no surprise merge commits).', inputSchema: s({ remote: { type: 'string' }, branch: { type: 'string' }, ffOnly: { type: 'boolean' } }) },
