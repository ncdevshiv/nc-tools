// Deterministic dual-implementation performance benchmark: the archived JS
// oracle server vs the primary Rust binary, both as opaque MCP-stdio
// processes, identical workload, identical corpus. No LLM involved.
//
// Usage: node benchmark/kernel-perf.mjs [outFile]
...
const out = resolve(process.argv[2] || join(root, 'benchmark', 'results', 'kernel-perf.json'));
const ITERS = 15;

const arms = {
  js: { cmd: process.execPath, args: [join(root, 'oracle', 'mcp', 'server.mjs')] },
  rust: { cmd: join(root, 'rust', 'target', 'release', 'nc-tools-mcp.exe'), args: [] },
};
