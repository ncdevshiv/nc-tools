// Benchmark harness: task x model -> fresh workspace -> Rust agent run (the
// nct-agent binary, which embeds its own kernel) -> real verifier -> transcript
// JSON with metrics. Every artifact is on disk; verification is independent of
// the agent (fresh workspace checks + journal forensics), no self-reporting.
...
const outDir = resolve(process.argv[2] || join(repoRoot, 'bench', 'results'));
const onlyTask = process.env.NCTOOLS_TASK;
const onlyTasks = (process.env.NCTOOLS_TASKS || '').split(',').map((s) => s.trim()).filter(Boolean);
// The agent is the Rust nct-agent binary (embeds its own kernel). Point
// NCTOOLS_AGENT_CMD at another build to A/B it; default is the release build.
const agentCmd = process.env.NCTOOLS_AGENT_CMD
  || join(repoRoot, 'target', 'release', process.platform === 'win32' ? 'nct-agent.exe' : 'nct-agent');

if (!baseURL || models.length === 0) {
  console.error('Usage: NCTOOLS_LLM_BASEURL=... NCTOOLS_LLM_APIKEY=... NCTOOLS_LLM_MODELS=model-a,model-b node bench/harness.mjs [outDir]');
  process.exit(2);
}
if (!existsSync(agentCmd)) {
  console.error(`agent binary not found: ${agentCmd}\nBuild it first: cargo build --release -p nct-agent`);
  process.exit(2);
}
...
for (const model of models) {
  for (const task of tasks) {
    ...
    // The agent runs the task in this workspace against its own embedded
    // kernel; this harness then verifies the resulting workspace with an
    // independent kernel process — same tasks, same verifier, fresh eyes.
    ...
    const result = report ?? {
      finalText: `agent failed (exit ${r.status}): ${String(r.stderr || '').slice(-300)}`,
      steps: 0, toolCalls: 0, errors: 0, usage: {}, stopped: 'agent_error',
    };
    const wallMs = Date.now() - started;
    // The verifier runs on an independent kernel rooted at the same workspace;
    // journal forensics read the on-disk journal the agent's own kernel wrote
    // (identical format: <root>/.nc-tools/journal.jsonl — protocol parity).
    const kernel = pooledKernel(root);
    let verdict;
    try {
