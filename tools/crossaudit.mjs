...
(async () => {
  ...
  try {
    ...
    const expected = ['agent.heartbeat','agent.list','agent.lock','agent.locks','agent.messages','agent.post','agent.register','agent.status','agent.unlock','batch.execute','code.symbols','env.get','env.list','env.set','fs.append','fs.copy','fs.delete','fs.list','fs.mkdir','fs.move','fs.read','fs.readMany','fs.readRange','fs.stat','fs.tree','fs.write','fs.writeMany','git.add','git.blame','git.branch','git.checkout','git.commit','git.diff','git.log','git.pull','git.push','git.status','net.cite','net.fetch','net.http','net.probePort','net.robots','net.search','net.verify','patch.apply','patch.applyMany','pkg.add','pkg.list','pkg.runScript','pkg.scripts','proc.kill','proc.list','proc.readOutput','proc.runScript','proc.spawn','proc.start','proc.status','proc.stop','proc.watch','search.files','search.grep','search.replace','search.semantic','sys.doctor','sys.journal','sys.listSnapshots','sys.rollback','sys.snapshot','sys.snapshotDiff','sys.workspace','test.run','text.diff'];
