...
pub fn register_debug_panic(k: &mut Kernel) {
    k.register("debug.panic", DEBUG_PANIC_DESC, nct_core::schema::schema_for::<DebugPanicArgs>(), std::sync::Arc::new(DebugPanicHandler));
}
...
impl Handler for DebugPanicHandler {
    fn call(&self, _k: &Kernel, args: &Value) -> Result<Value, ToolError> {
        ...
        let msg = a.message.unwrap_or_else(|| "debug.panic: intentional test panic".to_string());
        panic!("{msg}");
    }
}
