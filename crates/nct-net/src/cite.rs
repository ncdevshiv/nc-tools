...
pub fn register(k: &mut Kernel) {
    k.register("net.cite", CITE_DESC, nct_core::schema::schema_for::<CiteArgs>(), std::sync::Arc::new(CiteHandler));
    k.register("net.verify", VERIFY_DESC, nct_core::schema::schema_for::<VerifyArgs>(), std::sync::Arc::new(VerifyHandler));
}
...
fn citation_line(url: &str, title: &str, fetched_at: &str, content_hash: &str) -> String {
    ...
    format!("{title} ({url}, accessed {date}, sha256:{})", &content_hash[..8.min(content_hash.len())])
}
...
impl Handler for CiteHandler {
    fn call(&self, k: &Kernel, args: &Value) -> Result<Value, ToolError> {
        ...
        if let Some(id) = &a.id {
            ...
            let stored_hash = entry["contentHash"].as_str().unwrap_or_default().to_string();
            ...
            return match fetch_content(k, &fetch_args) {
                Ok(fresh) => {
                    let current = fresh["contentHash"].as_str().unwrap_or_default().to_string();
        ...
        if let Some(url) = &a.url {
            ...
            let (fetched, stale) = match fetch_content(k, &live_args) {
                Ok(f) => (f, false),
                Err(live_err) => {
                    let cached_args = json!({ "url": url, "allowPrivate": a.allowPrivate.unwrap_or(false) });
            ...
            let content_hash = fetched["contentHash"].as_str().unwrap_or_default().to_string();
            ...
            let fetched_at = fetched["fetchedAt"].as_str().unwrap_or_default().to_string();
            ...
            if !duplicate {
                ...
                let body = fetched["markdown"].as_str().unwrap_or_default().chars().take(200_000).collect::<String>();
                let entry = json!({
...
impl Handler for VerifyHandler {
    fn call(&self, k: &Kernel, args: &Value) -> Result<Value, ToolError> {
        ...
        let (url, markdown, cited_id) = if let Some(id) = &a.id {
            let sources = load_sources(&k.root);
            let entry = find_source(&sources, id).ok_or_else(|| {
                ToolError::with_hint("ERR_NOT_FOUND", format!("no such citation id: {id}"), json!({}))
            })?;
            (
            ...
            (url, fetched["markdown"].as_str().unwrap_or_default().to_string(), None)
        };
        ...
        if chunks.is_empty() {
            return Err(ToolError::new("ERR_BAD_INPUT", "no chunkable content in source"));
        }
        ...
        let _ = k.journal.append("net.verify", json!({
...
fn chunk_markdown(text: &str, max_chars: usize, min_chars: usize) -> Vec<String> {
    ...
    for line in text.lines() {
        ...
        if current.chars().count() + line.chars().count() + 1 > max_chars && current.chars().count() >= min_chars {
...
mod tests {
    ...
    fn chunker_respects_boundaries_and_caps() {
        let long = (0..200).map(|i| format!("Sentence number {i} with some filler words to add length.")).collect::<Vec<_>>().join("\n\n");
    ...
    fn cite_ids_differ_when_content_changes() {
        ...
        assert_eq!(cite_id("https://x.dev/a", "hash1"), a, "same source+content is idempotent");
    }
}
